...
function doInsert($jobid=0,$fileid=0) {
	...
	global $mydb; 
			...
			$applicant->AGE = $age;
			$applicant->PASS = sha1($_POST['password']);
			$applicant->create();


			$sql = "SELECT * FROM `tblcompany` c,`tbljob` j WHERE c.`COMPANYID`=j.`COMPANYID` AND JOBID = '{$jobid}' AND j.`JOBSTATUS`=1" ;
			$mydb->setQuery($sql);
...
function doRegister(){
	...
	if (isset($_POST['btnRegister'])) { 
			...
			message("You have successfully created Account in our Portal careerKenya. Click login in the upper right corner above to see Job Offers! All the best in your Job Search","success");
