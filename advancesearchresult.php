								...
								<tbody>
									<?php 
									 ...
									 $category = isset($_POST['CATEGORY']) ? $_POST['CATEGORY'] : '';
										...
										WHERE j.`COMPANYID`=c.`COMPANYID` AND j.`JOBSTATUS`=1 AND COMPANYNAME LIKE '%{$company}%' AND CATEGORY LIKE '%{$category}%' AND (`OCCUPATIONTITLE` LIKE '%{$search}%' OR `JOBDESCRIPTION` LIKE '%{$search}%' OR `QUALIFICATION_WORKEXPERIENCE` LIKE '%{$search}%')";
										$mydb->setQuery($sql);
